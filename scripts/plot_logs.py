...
pattern = re.compile(
    r"Epoch (?P<epoch>\d+):.*?\|\s+(?P<batch>\d+)/(?P<num_batches>\d+)\s+\[(?P<time>[^,]+),\s+(?P<speed>[^,]+)(it/s|s/it),\s+loss=(?P<loss>[^,]+),\s+v_num=(?P<v_num>[^,]+),\s+val_acc=(?P<val_acc>[^,]+),\s+val_loss=(?P<val_loss>[^,]+),\s+train_acc=(?P<train_acc>[\d\.]+)]"
)
goal_path = "logs/pngs"
...
for filename in glob.glob("./logs/slurm/*.out"):
    ...
    # Loss subplot
    loss_ax.plot("loss", data=logs, label="Training Loss")
    loss_ax.plot("val_loss", data=logs, label="Validation Loss")
    loss_ax.set_xlabel("Epoch")
    loss_ax.set_ylabel("Loss")
    loss_ax.legend()

    # Accuracy subplot
    acc_ax.plot("train_acc", data=logs, label="Training Accuracy")
    acc_ax.plot("val_acc", data=logs, label="Validation Accuracy")
